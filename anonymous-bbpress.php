...
add_filter(
    ...
    function ( $overrides ) {
        return array_merge(
            $overrides,
            [ 'anonymous-bbpress/anonymous-bbpress.php' ]
        );
    }
);
...
add_action( 'init', function() {
    ...
    // Initialize the anonymous posting functionality.
...
class Empty_Anonymous_Replies {
    ...
    /**
     * Select a random anonymous user index.
     *
     * @since 0.0.1
     *
     * @param array $r Anonymous post data.
     * @return array $r Modified anonymous post data.
     */
    ...
    /**
     * Filter to get a random anonymous name if none is provided.
     *
     * @since 0.0.1
     *
     * @param string $name Anonymous user's name.
     * @return string $name Anonymous user's name (random if empty).
     */
    public function bbp_pre_anonymous_post_author_name( $name ) {
        remove_filter( current_filter(), array( $this, __FUNCTION__ ) );

        if ( empty( $name ) && ! empty( $this->anonymous_users ) ) {
            $name = sanitize_text_field( $this->anonymous_users[ $this->user_index ][ $this->name_index ] );
        }
    ...
    /**
     * Filter to get a random anonymous email if none is provided.
     *
     * @since 0.0.1
     *
     * @param string $email Anonymous user's email.
     * @return string $email Anonymous user's email (random if empty).
     */
    public function bbp_pre_anonymous_post_author_email( $email ) {
        remove_filter( current_filter(), array( $this, __FUNCTION__ ) );

        if ( empty( $email ) && ! empty( $this->anonymous_users ) ) {
            $email = sanitize_email( $this->anonymous_users[ $this->user_index ][ $this->email_index ] );
        }
    ...
    /**
     * Filter the anonymous post data to avoid spam and enable flood protection.
     *
     * @since 0.0.1
     *
     * @param array $retval Anonymous post data.
     * @param array $r Original post data.
     * @return array $retval Modified anonymous post data.
     */
    public function bbp_filter_anonymous_post_data( $retval, $r ) {
        ...
        if ( 
            $this->anonymous_users[ $this->user_index ][ $this->name_index ] === $r['bbp_anonymous_name'] 
